...
Setup commands run asynchronously. `commandTimeoutMs` limits each command attempt,
while the instance's `options.timeout` limits the overall startup, including
retries. An instance timeout cancels the active command and any retry delay;
`stop()` waits for setup children to exit before removing their configuration.
