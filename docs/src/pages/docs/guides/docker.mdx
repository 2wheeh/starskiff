...
:::warning
**maroo is never published here.** Its node source is private, so it must not be redistributed. maroo stays local-only: inject your own image or binary (`Instance.marood({ image })` / `{ binary }`), or use a private image in a CI environment that allows it. A CI guard fails the build if any publish target mentions `maroo`/`marood`.
:::
...
:::note
The evmd image is the first of these and hasn't been published yet — its manifest digest is still `null`. Until the `publish-images` workflow runs, `Instance.evmd()`'s default image won't pull, so run `Instance.evmd({ binary: 'evmd' })` or point `image` at a locally-built tag in the meantime.
:::
