...
The inverse works too: instances that default to a binary (`simd`, `marood`, …) accept an `image` just the same, so you can point them at any image you have:

```ts
Instance.simd({ image: 'my-registry/simd:dev' });
```
...
| -------- | --------------- | ------ |
| `xplad` | Container | Official image, tracks mainnet |
| `evmd` | Container | **starskiff-published** image (cosmos/evm ships none) |
| [`simd`](/docs/instances/simd) | Binary | Reference app; simapp image only tags minor lines |
| others | Binary | No usable image yet (see [`config/binaries.json`](https://github.com/2wheeh/starskiff/blob/main/config/binaries.json)) |
