...
For an image that's built locally and never pushed to a registry, pass `pull: 'never'` — instead of attempting (and slowly failing) a registry pull when the image isn't present locally, it fails fast with an actionable error. Default is `'missing'` (pull only if absent locally); `pull` only matters alongside `image`.

```ts
Instance.marood({ image: 'marood:local', pull: 'never' });
```
